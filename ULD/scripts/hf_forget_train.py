...
def main(configs):
    ...
    #! setup trainer
    os.makedirs(logdir, exist_ok=True)
    strategy = trainer_config.get("strategy", "gpu")
    if strategy != "gpu":
        raise ValueError(
            f"RECAP assistant training supports only trainer.strategy=gpu, got {strategy!r}"
        )

    evaluation_strategy_arg = (
        {"eval_strategy": "steps"}
        if "eval_strategy"
        in inspect.signature(transformers.TrainingArguments.__init__).parameters
    ...
    training_args = transformers.TrainingArguments(
        ...
        optim=trainer_config.get('optim', "adamw_torch"),
        ...
        report_to=trainer_config.get('report_to', 'none'),
    ...
    if init_func is None:
        raise ValueError(
            f"Unsupported RECAP model mode: {model_mode.get('mode', 'base')!r}"
        )
    model = init_func(
        **model_config,
        **model_mode,
    ...
    if loss_requries_oracle(loss_config):
        ...
        if loss_config.get('oracle_source') == 'assistant_initialization':
            oracle_path = os.path.join(baseoutdir, 'fullmodel')
            if not os.path.isdir(oracle_path):
                raise FileNotFoundError(
                    "assistant-initialization oracle was not exported at "
                    f"{oracle_path}"
                )
        with NameTimer("Load oracle"):
            oracle_model = AutoModelForCausalLM.from_pretrained(
                oracle_path, torch_dtype=torch.bfloat16,
