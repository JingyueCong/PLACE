...
def AnswerMaskedKLLossFunc(
    model, input_ids, attention_mask, labels, oracle_model, **kwargs
):
    """Forward KL from a frozen reference, restricted to answer tokens.

    ``TorchDataset`` masks prompt and padding labels with ``-100``.  Applying
    the causal one-token shift here preserves the reference distribution only
    where an answer token is actually supervised.
    """
    if labels is None:
        raise ValueError("AnswerMaskedKLLossFunc requires labels")
    if oracle_model is None:
        raise ValueError("AnswerMaskedKLLossFunc requires a frozen oracle")

    outputs = model(input_ids=input_ids, attention_mask=attention_mask)
    model_log_probs = F.log_softmax(
        outputs.logits[..., :-1, :].contiguous(), dim=-1
    )
    shifted_labels = labels[..., 1:].contiguous()
    ...
    weights = valid.to(token_kl.dtype)
    return (token_kl * weights).sum() / weights.sum().clamp(min=1.0)
...
def FLATLossFunc(
    ...
    oracle_model=None, f_div='tv', **kwargs
):
