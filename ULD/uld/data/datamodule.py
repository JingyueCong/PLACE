...
class EqualForgetRetainSampler(Sampler):
    """Interleave RECAP fit rows and their counterfactual control rows."""
...
class TorchDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, data, tokenizer, conv_template, max_length=500, forget_length=None, retain_length=None, dpo_mode=False):
        ...
        if self.dpo_mode:
            refusal_path = Path(__file__).resolve().parents[2] / "data" / "refusal.jsonl"
            if not refusal_path.is_file():
                raise FileNotFoundError(
                    "DPO mode requires a refusal response file at "
                    f"{refusal_path}"
                )
            with refusal_path.open(encoding="utf-8") as handle:
                self.alternative_responses = [
                    json.loads(line) for line in handle if line.strip()
                ]
            if not self.alternative_responses:
                raise ValueError(f"No refusal responses found in {refusal_path}")
